 ...
 * {@link SecretStore} and the `{ backendUrl, userId, deviceId, companionId? }` record persisted in the
  ...
  try {
    ...
    for (;;) {
      ...
      if (result.kind === 'success') {
        ...
        auditLifecycle(deps.audit, { backendUrl, event: 'pair', detail: { deviceId, userId } }, write)
        write(`Paired with ${backendUrl}. Run '${brand().binary} connect' to set up your coding CLIs.\n`)
        return { ok: true }
      }
 ...
 * check the companion transport's `/connect` gates on: one authenticated `GET
...
export async function runPairWithToken(
  ...
  try {
    ...
    const record: PairedBackend = { backendUrl, userId, deviceId }
    deps.state.upsertPairedBackend(scope, record)
    auditLifecycle(deps.audit, { backendUrl, event: 'pair', detail: { deviceId, userId, method: 'token' } }, write)
    write(`Paired with ${backendUrl}. Run '${brand().binary} connect' to set up your coding CLIs.\n`)
    return { ok: true }
  } catch (err) {
    write(`Pairing failed: ${err instanceof Error ? err.message : 'unknown error'}\n`)
 ...
 * regardless so the companion can no longer authenticate, and the result copy is honest that the device
...
export async function runUnpair(scope: string, deps: UnpairDeps): Promise<UnpairResult> {
  ...
  const serverRevoked =
    bearer !== null ? await revokeBackendSession(backendUrl, bearer, deps.fetchFn ?? defaultFetch) : false
