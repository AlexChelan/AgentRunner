...
function isTerminalFrame(frame: RunEventMsg | RunConversationMsg): boolean {
  return frame.type === 'run.event' && (frame.event.type === 'done' || frame.event.type === 'error')
}
...
/** A running companion transport client: one held stream in, ordinary requests out. */
...
/**
 * Builds the companion's transport client.
 *
...
export function createStreamClient(deps: StreamClientDeps): StreamClient {
   ...
   *
   * The connect body reports the CLIs this companion has connected (tool id + auth-health) so the
  ...
  function presenceQuery(): string {
    ...
    if (connections) {
      parts.push(`connections=${encodeURIComponent(JSON.stringify(toConnectionStatus(connections)))}`)
    }
   ...
   *
   * Connect and disconnect instructions are left UNDELIVERED once shutdown has begun, so they
  ...
  async function applyRun(payload: unknown): Promise<void> {
    ...
    if (stopping) {
      buffer({
        type: 'run.event',
        runId: start.runId,
        event: { type: 'error', message: 'companion stopped before the run could start' }
      })
      return
    }
    try {
