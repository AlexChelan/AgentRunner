...
import { isConnectableToolId, type ConnectableToolId } from './connect'
...
export function createDisconnectRunner(deps: DisconnectRunnerDeps): DisconnectRunner {
  ...
  const execute = async (instruction: DisconnectInstruction, toolId: ConnectableToolId): Promise<void> => {
  ...
  return {
    handle(instruction): void {
      ...
      serializer.run(toolId, () =>
        execute(instruction, toolId).catch((err: unknown) => log(`disconnect runner error: ${String(err)}\n`))
      )
    }
  }
}
