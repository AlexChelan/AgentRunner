...
import { connectHeadless, isConnectableToolId } from "./connect";
import type { ConnectableToolId, HeadlessConnectOutcome } from "./connect";
...
export interface ConnectRunnerDeps {
	...
	/** The SCOPE the connection records are written under (an account scope, or the local pseudo-scope). */
	backendUrl: string;
	...
	/**
	 * The identity a managed install is group-shared with, set ONLY on a contained host: this runner
	 * installs as the root daemon, but the CLI children run as the unprivileged agent uid and could
	 * not otherwise traverse the `0700` managed tree to exec the binary.
	 */
	...
	/**
	 * The per-tool serializer that chains this runner's executes. SHARE one instance with the disconnect
	 * runner so a tool's connect and disconnect serialize against each other (the newest instruction wins);
	 * defaults to a private serializer so a standalone connect runner still serializes its own same-tool work.
	 */
	...
	const execute = async (
		...
		const outcome = await connect(
			toolId,
			{
				...
				log,
				...(deps.installAgent ? { installAgent: deps.installAgent } : {})
			},
		...
		handle(instruction): void {
			...
			serializer.run(toolId, () =>
				execute(instruction, toolId).catch((err: unknown) =>
					log(`connect runner error: ${String(err)}\n`)
				)
			);
		}
	};
