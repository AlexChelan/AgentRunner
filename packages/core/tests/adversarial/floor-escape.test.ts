...
import { buildRunnerRegistry } from "../../src/runtime/connect";
 ...
 * contained on this host, the docs may not claim it is, and the fix is in the adapter or in what the docs
 * claim - never in this file.
 *
 * OPENCODE AND HERMES ARE NO LONGER SUBJECTS, and their absence is the finding rather than a gap. Both
 * leaked here on 2026-07-29 (Hermes read the user's real `~/.ssh` and printed the key material). That
 * result was first answered with allow-and-disclose - the pair-time warning, the `status` line, the
 * per-run audit marker - and the product owner reversed it on 2026-08-02: a warning is not containment,
 * so both were removed from `CONNECTABLE_TOOL_IDS` instead. Neither has an adapter now, so there is no
 * floored run left to point at a canary. Should a future ACP release ship a real tool-restriction lever,
 * re-adding them here is the acceptance test for admitting them back.
 */
 ...
 */
async function runFloored(
	toolId: string,
	prompt: string
): Promise<{ haystack: string; ended: boolean }> {
	const appDataRoot = mkdtempSync(join(tmpdir(), "floor-appdata-"));
	mkdirSync(join(appDataRoot, "work"), { recursive: true });
	const registry = buildRunnerRegistry(appDataRoot);
