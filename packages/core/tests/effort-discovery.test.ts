...
import type { ConnectionRef, ModelInfo } from '@opencompanion/core-types'
import {
  applyAdvertisedEfforts,
  memoizeAdvertisedModels
} from '../src/adapters/agentic-run'
import { createClaudeCodeAdapter, type ClaudeAdapterDeps } from '../src/adapters/claude-code'
...
function openCodeAdapter(): RuntimeToolAdapter {
...
function hermesAdapter(): RuntimeToolAdapter {
...
describe('declared effort floors', () => {
  ...
  it('OpenCode and Hermes declare that effort reaches nothing on the path we drive', () => {
    // OpenCode's `run` has no effort flag at all. Hermes' is the PRE-DISCOVERY answer: its ACP session
    // decides whether a `thought_level` config option exists, and until one is probed the honest
    // declaration is that we know of no channel (see the Hermes adapter's own suite for the probed case).
    expect(openCodeAdapter().capabilities.effort).toEqual({ supported: false })
    expect(hermesAdapter().capabilities.effort).toEqual({ supported: false })
  })
})
