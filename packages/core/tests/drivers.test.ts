...
describe("claudeDriver", () => {
	...
	it("isolates the run from the user personal config (strict MCP + no filesystem settings)", async () => {
		...
		await drain(
			claudeDriver(
				claudeParams({ mcpServers: { runner: { type: "http", url: "http://127.0.0.1:1/t/mcp" } } })
			)
		);
	...
	it("hands a FLOORED run no file or shell tool, after the allow-list concatenation", async () => {
		...
		await drain(
			claudeDriver(
				claudeParams({
					...
					permissionMode: "read-only",
					allowedTools: ["mcp__agentrunner__lookup"]
				})
			)
		);
		...
		expect(options.allowedTools).toEqual(["mcp__agentrunner__lookup"]);
		for (const denied of [
			"Read",
			"Glob",
			"Grep",
	...
	it("loads the web tools as a floored run's only built-ins when egress is permitted", async () => {
		...
		await drain(
			claudeDriver(
				claudeParams({
					floored: true,
					allowedTools: ["mcp__agentrunner__lookup", "WebSearch", "WebFetch"]
				})
			)
		);
	...
	it("reports a FAILED tool from the id-matched tool_result, never a fabricated success", async () => {
		// The SDK stream carries the tool's real outcome on a later `user` message (a `tool_result`
		// block id-matched to the emitting `tool_use`, with `is_error`). Emitting `completed` off the
		// `tool_use` alone reported every failing tool as a success.
		const capture: { options?: unknown } = {};
		const query = fakeQuery(
			[
				{
		...
		expect(out.filter((m) => m.kind === "tool")).toEqual([
			{ kind: "tool", name: "Bash", status: "started", detail: "cat x" },
			{
				kind: "tool",
				name: "Bash",
				status: "failed",
	...
	it("settles a SUCCEEDING tool as completed, keeping the input summary as the detail", async () => {
		const capture: { options?: unknown } = {};
			...
			[
				{
					...
					message: {
						content: [
							{ type: "tool_use", id: "toolu_a", name: "Read", input: { file_path: filePath } }
						]
					}
				},
				{
		...
		const out = await drain(claudeDriver(claudeParams()));
		expect(out.filter((m) => m.kind === "tool")).toEqual([
			{ kind: "tool", name: "Read", status: "started", detail: filePath },
			{ kind: "tool", name: "Read", status: "completed", detail: filePath }
		]);
	});

	it("pairs each result to ITS OWN tool_use by id, out of order, and ignores unknown ids", async () => {
		// Two calls to the same tool settle by id, not by arrival order, so the failing one keeps its
		// own error text. A result for a tool_use this turn never emitted is dropped rather than
		// settling an unrelated card.
		const capture: { options?: unknown } = {};
		const query = fakeQuery(
			[
				{
				...
				},
				{
					...
					message: {
						role: "user",
						content: [
							{ type: "tool_result", tool_use_id: "t2", is_error: true, content: "two blew up" },
		...
		const out = await drain(claudeDriver(claudeParams()));
		expect(out.filter((m) => m.kind === "tool")).toEqual([
			{ kind: "tool", name: "Bash", status: "started", detail: "one" },
			{ kind: "tool", name: "Bash", status: "started", detail: "two" },
			{ kind: "tool", name: "Bash", status: "failed", detail: "two blew up" },
			{ kind: "tool", name: "Bash", status: "completed", detail: "one" }
		]);
	});
	...
	it("leaves a user message with no tool_result blocks alone", async () => {
		...
		const query = fakeQuery(
			[
				{
					type: "user",
					message: { role: "user", content: [{ type: "text", text: "a replayed prompt" }] }
				},
				{
					type: "result",
					subtype: "success",
		...
		const { claudeDriver } = makeDrivers({ query });
		const out = await drain(claudeDriver(claudeParams()));
		expect(out.some((m) => m.kind === "tool")).toBe(false);
	});
});

/** A streamed text delta, the cheapest message that counts as run activity. */
...
/**
 * A fake `query` that yields the supplied messages (optionally pausing `gapMs` before each) and then
 * HANGS forever - a wedged CLI that neither produces output nor exits, which is exactly the state the
 * inactivity watchdog exists to recover from.
 */
function hangingQuery(messages: unknown[], capture: { options?: unknown }, gapMs = 0): ClaudeQuery {
	return ((params: { prompt: string | AsyncIterable<unknown>; options?: unknown }) => {
		capture.options = params.options;
		return (async function* () {
...
/** Reads the abort controller the driver handed the SDK, so a test can assert the run was torn down. */
function capturedController(capture: { options?: unknown }): AbortController {
	const options = capture.options as { abortController?: AbortController } | undefined;
	if (!options?.abortController) throw new Error("the driver passed no abortController");
	return options.abortController;
}

/**
 * The inactivity watchdog. A wedged Claude run is otherwise UNBOUNDED: it holds its in-flight slot
 * forever (blocking the daemon self-updater) while producing nothing, and unlike Codex the SDK path had
 * no ceiling of its own.
 */
describe("claudeDriver - inactivity watchdog", () => {
	it("fires when the SDK goes silent, ending the run with an error and aborting the CLI", async () => {
		const capture: { options?: unknown } = {};
		const query = hangingQuery([textDelta("half an ans")], capture);
		const { claudeDriver } = makeDrivers({ query, claudeStallTimeoutMs: 25 });
		const out = await drain(claudeDriver(claudeParams()));
		expect(out[0]).toEqual({ kind: "text", text: "half an ans" });
		expect(out.at(-1)).toEqual({
			kind: "error",
			message: expect.stringContaining("stalled") as unknown as string
		});
		// The abort is what stops the wedged CLI from running on unseen after the run is reported failed.
		expect(capturedController(capture).signal.aborted).toBe(true);
	});

	it("resets on EVERY message, so a slow-but-producing run streams past the window", async () => {
		const capture: { options?: unknown } = {};
		// Six gaps of 10ms against a 25ms window: only a per-message reset gets this to its result.
		const query = hangingQuery(
			[
				textDelta("a"),
				textDelta("b"),
				textDelta("c"),
		...
		expect(out.at(-1)).toEqual({ kind: "done", usage: { inputTokens: 1, outputTokens: 1 } });
	});

	// A CLI that answered and is then slow to close its stream has already been reported to the user.
	// Firing the watchdog there would append an error part to a run that SUCCEEDED.
	it("treats silence AFTER the result as teardown, not a stall", async () => {
		const capture: { options?: unknown } = {};
		const query = hangingQuery(
			[
				{
		...
		const out = await drain(claudeDriver(claudeParams()));
		expect(out).toEqual([
			{ kind: "conversation", id: "sess-slow-close" },
			{ kind: "done", usage: { inputTokens: 2, outputTokens: 2 } }
		]);
	});

	it("stays silent when the run was already cancelled (a cancel is not a stall)", async () => {
		const capture: { options?: unknown } = {};
		const query = hangingQuery([], capture);
		const controller = new AbortController();
		const { claudeDriver } = makeDrivers({ query, claudeStallTimeoutMs: 25 });
		const out: AgenticDriverMessage[] = [];
		const run = (async () => {
			for await (const m of claudeDriver(claudeParams({ signal: controller.signal }))) out.push(m);
		})();
		controller.abort();
		await run;
		expect(out).toEqual([]);
...
class FakeAppServer extends EventEmitter {
	...
	private onData(chunk: string): void {
		this.buf += chunk;
		let nl: number;
		// The line-splitting idiom: assign the next newline index and test it in one step. The extra
		// parentheses are the documented way to signal the assignment is deliberate.
		// eslint-disable-next-line no-cond-assign -- see above
	...
	private respond(method: string, id: number): void {
		...
		if (method === "initialize") {
			this.push({
				jsonrpc: "2.0",
				id,
				result: { userAgent: "codex/0.142.3", codexHome: "/h/.codex" }
			});
...
describe("codexDriver", () => {
	...
	it("resumes a prior thread via thread/resume (never thread/start) when p.resume is set", async () => {
		...
		const { codexDriver } = makeDrivers({ spawnFn });
		await drain(
			codexDriver(cliParams({ binaryPath: "/usr/local/bin/codex", resume: "thread-77" }))
		);
	...
	it("sends the prompt as structured turn/start input, never as a spawn argument", async () => {
		...
		const { codexDriver } = makeDrivers({ spawnFn });
		await drain(
			codexDriver(cliParams({ binaryPath: "/usr/local/bin/codex", prompt: "--dangerous" }))
		);
	...
	it("spawns a FLOORED run with the whole filesystem denied, and no legacy sandbox tier", async () => {
		...
		const child = new FakeAppServer({ notifications: successNotifications() });
		const { spawnFn, callArgs } = fakeSpawn(child);
		// Pinned to a host that CAN confine the run, or there is no spawn to inspect: a floored codex run
		// is refused outright where the sandbox is not OS-enforced, so left to the real machine this
		// passes on macOS and throws that refusal on a Linux CI runner with no `bwrap`.
		const { codexDriver } = makeDrivers({ spawnFn, platform: "darwin" });
		await drain(
			codexDriver(
				cliParams({
		...
		const { args } = callArgs();
		expect(args).toContain('permissions.runner-confined.filesystem={"/" = "deny"}');
	...
	it("points CODEX_HOME at the isolated home when one is supplied (else leaves it unset)", async () => {
		...
		// servers; the terminal path passes none and keeps the user's own ~/.codex. A REAL directory,
		// because the driver re-authors that home's config.toml immediately before the spawn.
		const isoHome = mkdtempSync(join(tmpdir(), "drivers-iso-home-"));
	...
	it("threads MCP servers into -c mcp_servers overrides (auto-approved)", async () => {
		...
		await drain(
			codexDriver(
				cliParams({
					binaryPath: "/usr/local/bin/codex",
					mcpServers: { runner: { type: "http", url: "http://127.0.0.1:1/t/mcp" } }
				})
			)
		);
		const { args } = callArgs();
 ...
 */
function fakeModelQuery(models: unknown, capture: { options?: unknown } = {}): ClaudeQuery {
