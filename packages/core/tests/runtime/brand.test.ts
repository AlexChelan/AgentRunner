...
// A fake brand deliberately DIFFERENT from the boilerplate AgentRunner identity, so an assertion
...
describe("the engine brand seam", () => {
	it("defaults to the AgentRunner identity when no shell configures one", () => {
		expect(brand()).toEqual(DEFAULT_BRAND);
		expect(DEFAULT_BRAND.binary).toBe("agentrunner");
	...
	it("carries no wire identity: the frozen `runner` client id never lives on the brand", () => {
...
describe("the brand is read lazily, never at module scope", () => {
	it("a brand configured AFTER paths.ts was imported still steers appDataDir", async () => {
		...
		// into a rebranded runner's app-data path. Importing first and configuring second reproduces
		// that order exactly.
