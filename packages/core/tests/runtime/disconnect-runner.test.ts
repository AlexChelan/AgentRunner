...
import type { CliConnectionInfo, DisconnectInstruction, DisconnectResultBody } from '@opencompanion/protocol'
import { describe, expect, it } from 'vitest'
import { createDisconnectRunner, type DisconnectRunner, type DisconnectRunnerDeps } from '../../src/runtime/disconnect-runner'
import { createStateStore, type CliConnection, type StateStore } from '../../src/runtime/storage/state-store'
...
function makeRunner(
  ...
  const logs: string[] = []
  const state = createStateStore({ cwd: mkdtempSync(join(tmpdir(), 'companion-disconnect-runner-')) })
...
describe('disconnect runner', () => {
  it('removes a connected CLI and posts a disconnected result with a fresh connections snapshot', async () => {
    ...
    expect(calls.post).toEqual([
      { requestId: 'req-1', body: { toolId: 'claude-code', status: 'disconnected', connections: [] } }
    ])
  ...
  it('maps a thrown removal onto a failed result', async () => {
    const { runner, calls, logs } = makeRunner([], {
      readState: (): StateStore => {
        const store = createStateStore({ cwd: mkdtempSync(join(tmpdir(), 'companion-disconnect-fail-')) })
        return {
          ...store,
          removeConnection: () => {
