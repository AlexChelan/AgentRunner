...
import type { LocalTaskOverrideStore } from "../../src/runtime/local/task-overrides";
...
import type { LocalScheduleStore } from "../../src/runtime/local/schedule-store";
...
/**
 * A desktop-surfaced built-in schedule spec (fixture honesty: never the shipped web-only catalog entry). A
...
/** A user-schedule PUT body carrying a CRON cadence, with no `intervalMinutes` key at all. */
const cronScheduleBody = (over: Record<string, unknown> = {}): string =>
	JSON.stringify({
		name: "Nightly",
		prompt: "do the thing",
		cron: "0 9 * * *",
...
describe("startLocalDriveServer - advertised effort levels", () => {
	...
	it("accepts an OFF-LADDER schedule effort and round-trips it through the store", async () => {
		...
		const created = await send(socketPath, {
			method: "PUT",
			path: "/v1/schedules/new",
			token,
			body: scheduleBody({ effort: "ultra" })
		});
		...
		const listed = JSON.parse(
			(await send(socketPath, { method: "GET", path: "/v1/schedules", token })).text
		).schedules;
...
describe("startLocalDriveServer - schedules auth and caps", () => {
	it("404s every schedule route unauthed (the shared auth handler covers the new routes)", async () => {
		...
		expect(
			(await send(socketPath, { method: "PUT", path: "/v1/schedules/x", body: scheduleBody() }))
				.status
		).toBe(404);
		expect((await send(socketPath, { method: "DELETE", path: "/v1/schedules/x" })).status).toBe(
			404
		);
		expect(
			(await send(socketPath, { method: "POST", path: "/v1/schedules/x/run-now" })).status
		).toBe(404);
	});
...
describe("startLocalDriveServer - schedules CRUD", () => {
	it("creates a user schedule (returning a MINTED id), lists it, updates it in place, then deletes it", async () => {
		...
		const created = await send(socketPath, {
			method: "PUT",
			path: "/v1/schedules/throwaway-path-id",
			token,
			body: scheduleBody({ name: "First" })
		});
		expect(created.status).toBe(200);
		...
		expect(createdBody.schedule).toMatchObject({
			origin: "local",
			name: "First",
			prompt: "do the thing",
			intervalMinutes: 30,
		...
		const updated = await send(socketPath, {
			method: "PUT",
			path: `/v1/schedules/${mintedId}`,
			token,
			body: scheduleBody({ name: "Renamed" })
		});
		expect(updated.status).toBe(200);
		expect(JSON.parse(updated.text).schedule).toMatchObject({ id: mintedId, name: "Renamed" });
		expect(
			JSON.parse((await send(socketPath, { method: "GET", path: "/v1/schedules", token })).text)
				.schedules
		).toHaveLength(1);
	...
	it("surfaces a schedule run state on the list (the store is the source of truth)", async () => {
		...
		const created = schedules.upsertUser({
			name: "S",
			prompt: "p",
			intervalMinutes: 15,
			enabled: true
		});
		schedules.setRunState(created.id, {
			lastRunAt: 111,
	...
	it("400s an invalid user PUT: below-floor interval, empty name, malformed JSON, and an unsafe id", async () => {
		...
		expect(
			(
				await send(socketPath, {
					method: "PUT",
					path: "/v1/schedules/id1",
					token,
					body: scheduleBody({ intervalMinutes: 4 })
				})
			).status
		).toBe(400);
		expect(
		...
		expect(
			(await send(socketPath, { method: "DELETE", path: "/v1/schedules/a%2Fb", token })).status
		).toBe(400);
	});
	...
	it("dELETE of an unknown user id is an idempotent 200 (mirrors the chat DELETE posture)", async () => {
		...
		const res = await send(handle.socketPath, {
			method: "DELETE",
			path: "/v1/schedules/never-existed",
			token: handle.token
		});
		expect(res.status).toBe(200);
	});
	...
	it("is idempotent under a create retry: two PUTs to the same UUID path persist exactly one schedule", async () => {
		...
		const first = await send(socketPath, {
			method: "PUT",
			path: `/v1/schedules/${uuid}`,
			token,
			body: scheduleBody({ name: "Once" })
		});
...
describe("startLocalDriveServer - built-in enabled override", () => {
	it("lists a built-in with its effective (overridden) enabled and accepts ONLY { enabled } both directions", async () => {
		...
		const on = await send(socketPath, {
			method: "PUT",
			path: "/v1/schedules/digest",
			token,
			body: JSON.stringify({ enabled: true })
		});
		expect(on.status).toBe(200);
		expect(JSON.parse(on.text).schedule).toMatchObject({
			id: "digest",
			builtIn: true,
			enabled: true
		});
	...
	it("400s a full-shape PUT (or any extra key) on a built-in id, and 400s a DELETE of a built-in", async () => {
		...
		expect(
			(
				await send(socketPath, {
					...
					path: "/v1/schedules/digest",
					token,
					body: scheduleBody({ enabled: true })
				})
			).status
		).toBe(400);
		...
		expect(
			(await send(socketPath, { method: "DELETE", path: "/v1/schedules/digest", token })).status
		).toBe(400);
	});
});

describe("startLocalDriveServer - schedule cadences", () => {
	it("round-trips a CRON user schedule through PUT then GET, carrying no intervalMinutes", async () => {
		const { handle } = await start();
		const { socketPath, token } = handle;
		const created = await send(socketPath, {
			method: "PUT",
			path: "/v1/schedules/throwaway-path-id",
			token,
			body: cronScheduleBody({ timezone: "Asia/Tokyo" })
		});
		expect(created.status).toBe(200);
		const schedule = JSON.parse(created.text).schedule;
		...
		expect("intervalMinutes" in schedule).toBe(false);
	...
	it("400s a body carrying BOTH cadences, NEITHER, or a timezone without a cron", async () => {
		...
		const put = (body: string): Promise<{ status: number }> =>
			send(socketPath, { method: "PUT", path: "/v1/schedules/id1", token, body });
		expect((await put(cronScheduleBody({ intervalMinutes: 30 }))).status).toBe(400);
		expect((await put(JSON.stringify({ name: "N", prompt: "p", enabled: true }))).status).toBe(400);
		expect((await put(scheduleBody({ timezone: "Asia/Tokyo" }))).status).toBe(400);
	});
	...
	it("400s an unparseable cron, an EMPTY cron, and a bogus timezone", async () => {
		...
		const put = (body: string): Promise<{ status: number }> =>
			send(socketPath, { method: "PUT", path: "/v1/schedules/id1", token, body });
		expect((await put(cronScheduleBody({ cron: "not a cron" }))).status).toBe(400);
		...
		expect((await put(cronScheduleBody({ cron: "" }))).status).toBe(400);
		expect((await put(cronScheduleBody({ timezone: "Not/AZone" }))).status).toBe(400);
	});
	...
	it("400s a VALID cron past the length cap - a refusal of the value, never a 413", async () => {
		...
		const res = await send(handle.socketPath, {
			...
			path: "/v1/schedules/id1",
			token: handle.token,
			body: cronScheduleBody({ cron: LONG_VALID_CRON })
		});
		expect(res.status).toBe(400);
		expect(schedules.listUser()).toEqual([]);
	});
	...
	it("projects a built-in CRON spec with its cron and timezone, and no intervalMinutes", async () => {
		...
		const [listed] = JSON.parse(
			(await send(handle.socketPath, { method: "GET", path: "/v1/schedules", token: handle.token }))
				.text
		).schedules;
		expect(listed).toMatchObject({
			id: "cron-digest",
...
describe("startLocalDriveServer - the projected next run", () => {
	it("projects an ISO nextRunAt for every cadence, never in the past", async () => {
		...
		const schedules = createLocalScheduleStore(join(dir, "schedules"));
		const now = Date.now();
		const ran = schedules.upsertUser({
			name: "Ran",
			prompt: "p",
			intervalMinutes: 30,
			enabled: true
		});
		schedules.setRunState(ran.id, { lastRunAt: now - 10 * 60_000 });
		const fresh = schedules.upsertUser({
			name: "Fresh",
			prompt: "p",
			intervalMinutes: 30,
			enabled: true
		});
		const armed = schedules.upsertUser({
			name: "Armed",
			prompt: "p",
			cron: "0 9 * * *",
			timezone: "UTC",
		...
		schedules.setRunState(armed.id, { nextRunAtMs: armedAt });
		...
		// An interval schedule that HAS run projects from its last run.
		expect(byId.get(ran.id)?.nextRunAt).toBe(new Date(now - 10 * 60_000 + 30 * 60_000).toISOString());
		...
		// A cron schedule reports the instant the runner armed.
		expect(byId.get(armed.id)?.nextRunAt).toBe(new Date(armedAt).toISOString());
		for (const schedule of listed) {
			expect(Date.parse(schedule.nextRunAt)).toBeGreaterThanOrEqual(now);
		}
	});
	...
	it("omits nextRunAt entirely for a built-in cron the parser cannot project", async () => {
		...
		// than throw the whole read - the schedule itself still lists.
		const { handle } = await start({
	...
	it("writes NOTHING on a list: run-state.json is byte-identical across two GETs", async () => {
		...
		const schedules = createLocalScheduleStore(scheduleDir);
		const cron = schedules.upsertUser({
			name: "Cron",
			prompt: "p",
			cron: "*/5 * * * *",
			timezone: "UTC",
		...
		schedules.setRunState(cron.id, { lastRunAt: 1 });
		schedules.upsertUser({ name: "Interval", prompt: "p", intervalMinutes: 30, enabled: true });
		const runStateFile = join(scheduleDir, "run-state.json");
		...
		const get = (): Promise<{ status: number }> =>
			send(handle.socketPath, { method: "GET", path: "/v1/schedules", token: handle.token });
		expect((await get()).status).toBe(200);
		expect((await get()).status).toBe(200);
		expect(readFileSync(runStateFile, "utf8")).toBe(before);
		expect(readdirSync(scheduleDir).sort()).toEqual(["run-state.json", "user-schedules.json"]);
	});
});
	...
	it("fails SAFE with 503 on PUT and DELETE when the config read throws (never edits a built-in as a user schedule)", async () => {
		...
		expect(
			(
				await send(socketPath, {
					method: "PUT",
					path: "/v1/schedules/maybe-built-in",
					token,
					body: scheduleBody()
				})
			).status
		...
		expect(
			(await send(socketPath, { method: "DELETE", path: "/v1/schedules/maybe-built-in", token }))
				.status
		).toBe(503);
	});
});
...
describe("startLocalDriveServer - run-now", () => {
	it("maps the runner arms started/busy/unknown/failed to 202/409/404/500", async () => {
		for (const [arm, status] of [
			...
			const res = await send(handle.socketPath, {
				method: "POST",
				path: "/v1/schedules/sched-1/run-now",
				token: handle.token
			});
	...
	it("400s a run-now on an unsafe id before the runner is consulted", async () => {
		...
		const res = await send(handle.socketPath, {
			method: "POST",
			path: "/v1/schedules/a%2Fb/run-now",
			token: handle.token
		});
		expect(res.status).toBe(400);
