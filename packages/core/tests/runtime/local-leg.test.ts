...
import { startLocalLeg, type LocalLeg } from '../../src/runtime/local/local-leg'
...
function deps(): {
  ...
  const appDataRoot = mkdtempSync(join(tmpdir(), 'companion-local-leg-'))
...
function refused(socketPath: string): Promise<boolean> {
  return new Promise((resolve) => {
    const req = request({ socketPath, path: '/v1/health', method: 'GET', headers: { host: DRIVE_HOST } }, (res) => {
      res.resume()
...
function drive(leg: LocalLeg, method: string, path: string, body?: string): Promise<{ status: number; text: string }> {
  return new Promise((resolve, reject) => {
    const headers: Record<string, string> = { host: DRIVE_HOST, authorization: `Bearer ${leg.token}` }
...
describe('startLocalLeg (composable local executor leg)', () => {
  ...
  it('serves the runtime adapter model catalog over GET /v1/tools/<toolId>/models (picker wire shape)', async () => {
    ...
    const adapter: RuntimeToolAdapter = {
      ...
      authStatus: async (): Promise<AuthStatus> => ({ authenticated: true, mode: 'subscription' as const }),
      listModels: async (conn: ConnectionRef): Promise<ModelInfo[]> => {
        asked.push(conn)
        return [
          { id: 'claude-fable-5', label: 'Claude Fable 5', source: 'registry', recommended: true, contextWindow: 500000 },
    ...
    expect(asked).toEqual([{ id: 'companion-claude-code', toolId: 'claude-code', authMode: 'subscription' }])
  })
  ...
  it('stands the adapter declared floor in for a model discovery never reached', async () => {
    ...
    const adapter: RuntimeToolAdapter = {
      ...
      capabilities: {
        ...FAKE_CAPABILITIES,
        effort: { supported: true, levels: ['low', 'medium', 'high', 'xhigh', 'max'], canDisable: false }
      },
      detect: async (): Promise<DetectResult> => ({ installed: true }),
      authStatus: async (): Promise<AuthStatus> => ({ authenticated: true, mode: 'subscription' as const }),
      listModels: async (): Promise<ModelInfo[]> => [
        { id: 'gpt-5.6-sol', source: 'tool', effortLevels: ['low', 'medium', 'high', 'xhigh', 'ultra'] },
        { id: 'gpt-5.4', source: 'registry' }
      ],
      run: (): never => {
        throw new Error('never run in this test')
    ...
    expect(JSON.parse(body.text)).toEqual({
      models: [
        ...
        { id: 'gpt-5.6-sol', name: 'gpt-5.6-sol', effortLevels: ['low', 'medium', 'high', 'xhigh', 'ultra'] },
  ...
  it('contributes no floor for an adapter that declares it has no effort channel at all', async () => {
    ...
    d.registry = {
      getAdapters: () => [adapter],
      getAdapter: (id) => (id === 'opencode' ? adapter : undefined),
      requireAdapter: () => adapter
    }
    const leg = await startLocalLeg(d)
    running.push(leg)

    const body = await drive(leg, 'GET', '/v1/tools/opencode/models')

    expect(JSON.parse(body.text)).toEqual({ models: [{ id: 'some-model', name: 'some-model' }] })
  })

  it('serves the REAL Hermes adapter’s ACP-advertised models (the desktop picker’s only source)', async () => {
    // End-to-end for the ACP catalog: the adapter's session probe -> listModels -> this route -> the
    // picker's wire shape. Wired with the real `createHermesAdapter` (only the session probe is faked)
    // so a regression anywhere in that chain fails here, not just in the adapter's own unit test.
    const d = deps()
    const adapter = createHermesAdapter({
      ...
      resolveBinary: () => join(tmpdir(), 'bin', 'hermes'),
      loadApiKey: () => null,
    ...
    d.registry = {
      getAdapters: () => [adapter],
      getAdapter: (id) => (id === 'hermes' ? adapter : undefined),
      requireAdapter: () => adapter
    }
    const leg = await startLocalLeg(d)
    running.push(leg)

    const body = await drive(leg, 'GET', '/v1/tools/hermes/models')

    expect(body.status).toBe(200)
    // No effort ladder: this session advertises no `thought_level`, and the adapter's floor stays
    // `{ supported: false }` - so the picker hides the control exactly as it does today.
    expect(JSON.parse(body.text)).toEqual({
      models: [
        { id: 'openrouter:anthropic/claude-opus-5', name: 'anthropic/claude-opus-5', recommended: true },
        { id: 'openrouter:openai/gpt-5.6-sol', name: 'openai/gpt-5.6-sol' }
      ]
    })
  })

  it('carries a Hermes session’s advertised reasoning ladder onto the catalog route', async () => {
    const d = deps()
    const adapter = createHermesAdapter({
      driver: async function* () {
        throw new Error('never run in this test')
      ...
      listSession: async () => ({
        models: [{ id: 'openrouter:openai/gpt-5.6-sol', name: 'openai/gpt-5.6-sol' }],
        thoughtLevel: {
          id: 'reasoning_effort',
    ...
    expect(JSON.parse((await drive(leg, 'GET', '/v1/tools/hermes/models')).text)).toEqual({
      models: [
        {
  ...
  it('serves an EMPTY catalog for a connectable CLI the registry has no adapter for', async () => {
    // An empty list (never a throw/500) keeps the picker functional: buildCompanionModels renders a
...
describe('startLocalLeg - CLI catalog + in-app connect', () => {
  it('GET /v1/tools/catalog reports each connectable CLI live install/auth/connected state', async () => {
    const d = deps()
    // claude-code already connected locally; hermes installed + signed in but NOT connected here (the exact
    // shape a CLI signed in for a paired backend but never connected locally lands in - the invisible case).
    d.readState().upsertConnection('local', { toolId: 'claude-code', source: 'reused', authHealth: 'healthy' })
    d.registry = registryOf({
      'claude-code': fakeAdapter('claude-code', { installed: true, authenticated: true }),
      hermes: fakeAdapter('hermes', { installed: true, authenticated: true })
    })
    const leg = await startLocalLeg(d)
    running.push(leg)
    ...
    expect(Object.keys(byTool).sort()).toEqual(['claude-code', 'codex', 'hermes', 'opencode'])
    expect(byTool['claude-code']).toMatchObject({ installed: true, authenticated: true, connected: true })
    expect(byTool['hermes']).toMatchObject({ installed: true, authenticated: true, connected: false })
    // A connectable CLI the registry has no adapter for reports not-installed (never a throw).
    expect(byTool['codex']).toMatchObject({ installed: false, authenticated: false, connected: false })
  })

  it('POST /v1/tools/<id>/connect records a local connection so /v1/tools then lists it', async () => {
    const d = deps()
    d.registry = registryOf({ hermes: fakeAdapter('hermes', { installed: true, authenticated: true }) })
    const leg = await startLocalLeg(d)
    running.push(leg)
    ...
    // The in-app connect records it under the local scope (no interactive login: hermes is already signed in).
    const connectRes = await drive(leg, 'POST', '/v1/tools/hermes/connect')
    ...
    expect(JSON.parse((await drive(leg, 'GET', '/v1/tools')).text)).toEqual({
      tools: [{ toolId: 'hermes', authHealth: 'healthy', images: false }]
    })
  })

  it('POST /v1/tools/<id>/connect reports needs-login for an installed-but-signed-out CLI, recording nothing', async () => {
    const d = deps()
    d.registry = registryOf({ hermes: fakeAdapter('hermes', { installed: true, authenticated: false }) })
    const leg = await startLocalLeg(d)
    running.push(leg)
