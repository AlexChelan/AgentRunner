...
describe("account scope grammar", () => {
	...
	it("canonicalizes the url on the way in, so cosmetic variants map to one scope", () => {
		expect(accountScope("https://App.test/api/", "user-1")).toBe(
			accountScope("https://app.test/api", "user-1")
		);
	});

	it("round-trips", () => {
		const scope = accountScope("https://app.test/api", "user-1");
		expect(parseAccountScope(scope)).toEqual({
			backendUrl: "https://app.test/api",
			userId: "user-1"
		});
	});

	it("keeps two users on one backend distinct", () => {
		expect(accountScope("https://app.test/api", "a")).not.toBe(
			accountScope("https://app.test/api", "b")
		);
	});
	...
	it("recovers the backend url from either scope form", () => {
		expect(scopeBackendUrl(accountScope("https://app.test/api", "u1"))).toBe(
			"https://app.test/api"
		);
