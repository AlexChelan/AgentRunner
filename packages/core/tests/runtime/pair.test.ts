...
function harness() {
	const dir = mkdtempSync(join(tmpdir(), "runner-pair-"));
...
describe("variant-tolerant unpair (resolves the actual stored key in both store states)", () => {
	it("legacy raw-keyed store: unpair by a canonical variant removes the raw record + its bearer", async () => {
		...
		const resolved = await resolveBackendScope("https://app.com/api", undefined, h.state, {
			interactive: false
		});
	...
	it("canonical store: unpair by yet another URL variant removes the canonical record + its bearer", async () => {
		...
		const resolved = await resolveBackendScope("https://app.com:443/api", undefined, h.state, {
			interactive: false
		});
...
describe("bearerKey", () => {
	...
	it("gives two accounts on ONE backend two different bearer entries", () => {
		...
		expect(bearerKey(accountScope(backend, "user-a"))).not.toBe(
			bearerKey(accountScope(backend, "user-b"))
		);
	});
});
...
describe("two SaaS logins on one backend pair as two accounts", () => {
	...
	it("lets a second user pair the same backend without evicting the first", async () => {
		...
		await runPairWithToken(
			{ backendUrl: BACKEND, token: "tok-a" },
			{ ...deps, fetchFn: sessionFetchFor("user-a") }
		);
		await runPairWithToken(
			{ backendUrl: BACKEND, token: "tok-b" },
			{ ...deps, fetchFn: sessionFetchFor("user-b") }
		);
		...
		expect(
			h.state
				...
				.sort()
		).toEqual([accountScope(BACKEND, "user-a"), accountScope(BACKEND, "user-b")].sort());
...
/**
 * The message a failed device-authorization request gives the user.
 *
 * `pnpm cli pair --url http://localhost:3000` (the SITE origin, not the API base) answered
 * "device authorization request failed (HTTP 404)", which names nothing the user can act on - and the
 * mistake it reports is the single most likely one, since both URLs look equally plausible.
 */
describe("describeDeviceCodeFailure", () => {
	it("tells the user the URL must be the API base, and shows what it called", () => {
		const message = describeDeviceCodeFailure(404, "http://localhost:3000/auth/device/code");

		expect(message).toContain("http://localhost:3000/auth/device/code");
		expect(message).toContain("pass the API base");
		expect(message).toContain("http://localhost:3000/api");
	});

	// Any other status means the endpoint IS there and answered, so the URL is not the thing to
	// change and pointing at it would misdirect.
	it("keeps the plain report for a status that is not a missing endpoint", () => {
		for (const status of [400, 401, 429, 500]) {
			const message = describeDeviceCodeFailure(
				status,
				"http://localhost:3000/api/auth/device/code"
			);
			expect(message).toBe(`device authorization request failed (HTTP ${status})`);
			expect(message).not.toContain("API base");
		}
	});
});
