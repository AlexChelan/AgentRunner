import { RunStartSchema, type ReasoningEffort } from '@opencompanion/protocol'
...
import { composeLocalRun, type ComposeLocalRunOpts } from '../../src/runtime/local/compose-local-run'
...
describe('composeLocalRun', () => {
  ...
  it('carries attached images onto the RunStart as inputImages', () => {
    const images = [{ dataUrl: 'data:image/jpeg;base64,AAAA', mediaType: 'image/jpeg', width: 800, height: 600 }]
  ...
  it('never leaks an env key name into the refused list (names only)', () => {
    const out = composeLocalRun(
      base({ localMcpServers: { secretSrv: { type: 'stdio', command: 'srv', envKeys: ['API_KEY', 'TOKEN'] } } })
    )
  ...
  it('treats an empty envKeys array as env-less (passes through)', () => {
    const out = composeLocalRun(base({ localMcpServers: { d: { type: 'stdio', command: 'srv', envKeys: [] } } }))
  ...
  it('threads model, effort, and conversation resume onto start', () => {
    ...
    const out = composeLocalRun(base({ modelId: 'claude-sonnet-5', effort, conversationId: 'conv-1' }))
