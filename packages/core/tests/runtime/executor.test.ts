...
function appDataRoot(): string {
	return mkdtempSync(join(tmpdir(), "runner-exec-"));
}
 ...
 */
function wireDelivered(
	extra: Record<string, unknown>,
	overrides: Partial<RunStart> = {}
): RunStart {
...
function fakeSession(
	drive: (onEvent: (e: DriveEvent, runId: string) => void, runId: string) => void
) {
...
describe("executor", () => {
	...
	it("forwards the runtime conversation (SDK session id) UP instead of dropping it", () => {
		...
		exec.start(start({ runId: "dispatch-r3" }), {
			onEvent: () => {},
			onConversation: (m) =>
				conversations.push({ runId: m.runId, conversationId: m.conversationId }),
			onToolCall: async () => undefined,
			onClose: () => {}
		});
	...
	it("auto-APPROVES an unattended permission-request, and DISCLOSES what it allowed", () => {
		...
		const { sm, permissionResponses } = fakeSession((onEvent, runId) => {
			onEvent(
				{ type: "permission-request", requestId: "perm-1", toolName: "Bash", input: {} },
				runId
			);
			onEvent({ type: "done" }, runId);
		});
		...
		// The unattended runner has no approver, so it auto-APPROVES rather than auto-denying - a deny
		// would silently refuse every write and make the runner a read-only agent (no parity).
	...
	it("drives a dispatched run at the floor even under the most permissive requested policy", () => {
		...
		const exec = createExecutor(makeDeps({ sessionManager: smSpy }));
	...
	it("runs a dispatched CLI through to its terminal event", () => {
	...
	it("serves web tools over loopback MCP and proxies a manifest tool execute as a tool.call UP", async () => {
		...
		const exec = createExecutor(
			makeDeps({ sessionManager: smSpy, serveTools, shouldServe: () => true })
		);
		const proxiedCalls: { name: string }[] = [];
		exec.start(
			start({ webToolManifest: [{ name: "knowledge_search", inputSchema: { type: "object" } }] }),
			{
				onEvent: () => {},
		...
		expect(seenReq?.mcpServers?.[brand().binary]).toEqual({
			type: "http",
			url: "http://127.0.0.1:5/tok/mcp"
		});
	});
	...
	it("honors a cancel that arrives while serveTools() is still pending: closes the handle, never starts, audits cancelled (I12)", async () => {
		...
		const serve: {
			resolve:
				| ((v: { spec: { type: "http"; url: string }; close: () => Promise<void> }) => void)
				| null;
		} = { resolve: null };
		const { sm } = fakeSession(() => {});
		const smSpy = {
		...
		exec.start(
			start({
				runId: "pending-run",
				webToolManifest: [{ name: "k", inputSchema: { type: "object" } }]
			}),
			{
				onEvent: () => {},
				onToolCall: async () => undefined,
...
describe("executor audit", () => {
	...
	it("refuses the run when the dispatched append throws: never starts the CLI, emits a terminal error, logs the cause (fail-closed)", () => {
		...
		const onClose = vi.fn();
		const exec = createExecutor(
			makeDeps({ sessionManager: sm, audit, log: (l) => warnings.push(l) })
		);
	...
	it("refuses BEFORE serving web tools when the dispatched append throws (no loopback MCP, no CLI)", () => {
		...
		const exec = createExecutor(
			makeDeps({ sessionManager: sm, audit, serveTools, shouldServe: () => true })
		);
	...
	it("records a failed terminal entry carrying the error message on an error outcome", () => {
		const { sm } = fakeSession((onEvent, runId) =>
			onEvent({ type: "error", message: "boom" }, runId)
		);
		const { audit, appends } = recordingAudit();
		const exec = createExecutor(makeDeps({ sessionManager: sm, audit }));
		exec.start(start(), {
	...
	it("refuses a DISPATCH run (origin derived) when the origin policy denies dispatch: refused carries the origin tag", () => {
		...
		expect(appends[0]).toMatchObject({
			event: "refused",
			runId: "r-disp",
			detail: { origin: "site-audit", reason: "origin_denied" }
		});
	});
