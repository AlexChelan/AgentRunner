...
import { writeJsonFileAtomic } from "../../src/runtime/local/atomic-file";
import { isRecord } from "../../src/runtime/local/is-record";

/** A fresh temp dir under the OS temp root for one test's atomic writes. */
function dir(): string {
	return mkdtempSync(join(tmpdir(), "runner-atomic-file-"));
}
