...
function stateWith(urls: string[]): StateStore {
  ...
  urls.forEach((backendUrl, i) => store.upsertPairedBackend(backendUrl, { backendUrl, deviceId: `d${i}`, userId: 'u1' }))
  return store
}
...
describe('findPairedBackend', () => {
  it('resolves the exact key, a canonical variant, and returns the stored record (canonical store)', () => {
    ...
    expect(findPairedBackend('https://App.com/api/', state)?.backendUrl).toBe('https://app.com/api')
    expect(findPairedBackend('https://app.com:443/api', state)?.backendUrl).toBe('https://app.com/api')
  })
...
describe('canonicalizeBackendUrl', () => {
  it.each([
    // [input, expected, why]
    ['https://app.com/api/', 'https://app.com/api', 'strips a trailing slash but KEEPS the base path'],
    ['https://app.com/api///', 'https://app.com/api', 'strips repeated trailing slashes'],
    ['https://App.COM/api', 'https://app.com/api', 'lowercases the host (scheme is lowered by URL)'],
...
describe('findPairedScopes', () => {
  ...
  it('matches a textual variant of the url, and a legacy bare-url key', () => {
    ...
    expect(findPairedScopes('https://legacy.example/api/', state)[0]?.scope).toBe('https://legacy.example/api')
  })
})
...
describe('resolveBackendScope', () => {
  it('returns the scope of the sole pairing when nothing narrows it', async () => {
    ...
    await expect(resolveBackendScope(undefined, undefined, state, { interactive: false })).resolves.toBe(
      accountScope('https://only.example', 'user-a')
    )
  })
  ...
  it('resolves the one account a --user names when several share a backend', async () => {
    ...
    await expect(resolveBackendScope('https://app.example/api', 'user-b', state, { interactive: false })).resolves.toBe(
      accountScope('https://app.example/api', 'user-b')
    )
  })
  ...
  it('returns the CANONICAL url for an explicit --url that matches no pairing', async () => {
    ...
    const state = stateWithAccounts([['https://app.example/api', 'user-a']])
    await expect(
      resolveBackendScope('https://Absent.Example:443/api/', undefined, state, { interactive: false })
  ...
  it('throws the pair hint when nothing is paired at all', async () => {
    ...
    await expect(resolveBackendScope(undefined, undefined, state, { interactive: false })).rejects.toThrow(
      /Not paired with any backend/
    )
  })
})
