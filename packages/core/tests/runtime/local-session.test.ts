...
function fixtures(): { appDataRoot: string; readState: () => StateStore; secrets: SecretStore } {
  const appDataRoot = mkdtempSync(join(tmpdir(), 'companion-local-'))
...
function twoAdapterRegistry(
  a: { toolId: string; onRun: (req: RuntimeRunRequest, emit: (event: RuntimeRunEvent) => void) => void },
  b: { toolId: string; onRun: (req: RuntimeRunRequest, emit: (event: RuntimeRunEvent) => void) => void }
 ...
 */
function imageCapabilityRegistry(): { registry: AgentRuntimeRegistry; runReqs: RuntimeRunRequest[] } {
  const runReqs: RuntimeRunRequest[] = []
...
describe('createLocalSession', () => {
  it('composes and dispatches a chat: work/local/<productId> cwd, secrets deny-read, resolved CLI', () => {
    ...
    const { registry, runReqs } = recordingRegistry('codex', (_req, emit) => emit({ type: 'done' }))
  ...
  it('warns about an env-needing stdio MCP server and still starts the run without it', () => {
    ...
    state.upsertMcpServer(LOCAL_SCOPE, 'needs-env', { type: 'stdio', command: '/bin/tool', envKeys: ['API_KEY'] })
  ...
  it('falls back to the config defaultCli and defaultModel when the call omits them', () => {
    ...
    const session = createLocalSession({
      ...
      secrets,
      audit,
      config: () => ({ productId: 'demo', productName: 'Demo', defaultCli: 'codex', defaultModel: 'gpt-5-codex' }),
      write: () => {}
    })
  ...
  it('prefers the call cli/modelId over the config defaults', () => {
    ...
    const session = createLocalSession({
      ...
      secrets,
      audit,
      config: () => ({ productId: 'demo', productName: 'Demo', defaultCli: 'claude-code', defaultModel: 'unused' }),
      write: () => {}
    })
...
describe('createLocalSession.startScheduled', () => {
  ...
  function collector(): { onDone: (outcome: LocalScheduleOutcome | null, text: string) => void; calls: Array<[LocalScheduleOutcome | null, string]> } {
  ...
  it('stamps origin = scheduleId on the dispatched audit and starts the CLI after auditing', () => {
    ...
    session.startScheduled({ scheduleId: 'sched-1', prompt: 'go', cli: 'codex', onDone: done.onDone })
  ...
  it('classifies a policy-denied scheduled run as refused (scheduleId drives the derived kind)', () => {
    ...
    expect(refused?.detail).toMatchObject({ scheduleId: 'sched-1', origin: 'sched-1', reason: 'origin_denied' })
  ...
  it('classifies a non-denied error as failed', () => {
    const { appDataRoot, readState, secrets } = fixtures()
    connect(readState, 'codex')
    const { registry } = recordingRegistry('codex', (_req, emit) => emit({ type: 'error', message: 'boom' }))
    const { audit } = recordingAudit()
    const session = createLocalSession({
      appDataRoot,
      registry,
  ...
  it('settles failed on an unknown connection', () => {
    ...
    const done = collector()
    session.startScheduled({ scheduleId: 'sched-1', prompt: 'go', cli: 'claude-code', onDone: done.onDone })
    expect(done.calls).toEqual([['failed', '']])
  })
