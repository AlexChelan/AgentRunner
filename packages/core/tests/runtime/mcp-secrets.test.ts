...
import { collectMcpEnv, deleteMcpEnv, mcpEnvKey, readMcpEnv, writeMcpEnv } from '../../src/runtime/mcp-secrets'
...
function secretStore(): SecretStore {
  const dir = join(mkdtempSync(join(tmpdir(), 'companion-mcpsecrets-')), 'secrets')
...
describe('mcpEnvKey', () => {
  it('is filesystem-safe and scopes a server to ONE backend', () => {
    ...
    expect(mcpEnvKey('https://Buyer.example:8443/a b?c=1', 'linear')).toMatch(/^mcp-env-[0-9a-f]{32}$/)
...
describe('writeMcpEnv / readMcpEnv', () => {
  it('round-trips the values through the encrypted store', () => {
    ...
    expect(readMcpEnv(secrets, BACKEND, 'linear')).toEqual({ LINEAR_KEY: 'lin_secret_abc', TEAM: 'core' })
