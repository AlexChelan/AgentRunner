...
/**
 * Whether `script(1)` is on PATH. {@link spawnPty} shells out to it on both dialects, so a slim
 * container without it would fail every case below rather than simply not having them.
 */
...
// CONDITIONAL REGISTRATION, not a skip: a test that is not registered is not reported, and the suite
// summary's "0 skipped" is the contract this repo ships.
if (isPtyAvailable() && hasScript()) {
	describe("spawnPty", () => {
