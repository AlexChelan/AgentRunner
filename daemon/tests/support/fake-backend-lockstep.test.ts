 ...
 * and the runtime's own backend suites, and the runner export copies each package independently - so a
 ...
 * has. This is the same guard shape as `packages/config/tests/runner-brand-lockstep.test.ts`.
 *
...
/**
 * The twin's path. The exporter renames the trees (`apps/runner` -> `daemon`, `packages/agent-core` ->
 * `packages/core`), so the sibling is DISCOVERED rather than assumed - a hardcoded monorepo path would
 * ENOENT in the export and fail `runner:verify`, i.e. the whole publish.
 *
